...
const DocumentScanner: React.FC<DocumentScannerProps> = ({ onCapture, onClose }) => {
  ...
  const capturePhoto = async () => {
    ...
    // 1. BURST FLASH: Turn on torch briefly
    await applyTorch(true);
    
    // UI Flash Feedback
    setFlashUI(true);
    
    // Wait for auto-exposure to adjust briefly
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-black flex flex-col items-center justify-center overflow-hidden h-[100dvh] w-screen left-0 top-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-[1100]">
        <div className="w-[85%] max-w-[450px] h-[60%] border-2 border-white/40 rounded-[2.5rem] relative overflow-hidden">
          <div className="absolute top-0 left-0 w-16 h-16 border-t-[6px] border-l-[6px] border-indigo-500 rounded-tl-[2rem]"></div>
          <div className="absolute top-0 right-0 w-16 h-16 border-t-[6px] border-r-[6px] border-indigo-500 rounded-tr-[2rem]"></div>
          <div className="absolute bottom-0 left-0 w-16 h-16 border-b-[6px] border-l-[6px] border-indigo-500 rounded-bl-[2rem]"></div>
          <div className="absolute bottom-0 right-0 w-16 h-16 border-b-[6px] border-r-[6px] border-indigo-500 rounded-br-[2rem]"></div>
          
          <div className="absolute inset-x-0 h-[3px] bg-indigo-400 animate-[scan_2s_infinite] shadow-[0_0_15px_#6366f1] z-10"></div>
        </div>
        <p className="mt-8 text-white text-[10px] font-black uppercase tracking-[0.2em] bg-black/40 px-6 py-2 rounded-full backdrop-blur-md">
          Burst Flash Scan: Position Receipt Inside Frame
        </p>
      </div>

      <div className="absolute bottom-0 inset-x-0 h-40 bg-gradient-to-t from-black/80 to-transparent flex items-center justify-around px-10 pb-10 z-[1150]">
        <button onClick={onClose} className="w-14 h-14 flex items-center justify-center rounded-2xl bg-white/10 text-white">
        ...
        <button 
          ...
          className="w-24 h-24 rounded-full border-4 border-white p-1 bg-transparent group active:scale-95 transition-all shadow-[0_0_20px_rgba(255,255,255,0.3)]"
        >
